...
public class LevelEditor : MonoBehaviour
{
	...
	public void OnFire()
	{
		...
		if (Physics.Raycast(mainCamera.ScreenPointToRay(mousePosition), out RaycastHit hitInfo) && hitInfo.collider.gameObject.CompareTag("Editable"))
		{
			...
			hitOffset = activeHitObject.transform.position - ray.GetPoint(distance);
	...
	private void Update()
	{
		...
		if (activeHitObject != null && Mouse.current.leftButton.isPressed)
		{
			...
			if (mouseScroll != 0)
			{
				...
				activeHitObject.transform.Rotate(activeHitObject.transform.InverseTransformDirection(Vector3.forward), mouseScroll * 7.5f);
